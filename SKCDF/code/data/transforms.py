...
class RandomCrop_Batch(object): #2,80,160,160
    '''
    Crop randomly the image in a sample
    Args:
    output_size (int): Desired output size
    ...
    def __call__(self, sample):
        image = sample['image']
        padding_flag = image.shape[1] <= self.output_size[1] or image.shape[2] <= self.output_size[2] or image.shape[
            3] <= self.output_size[3]
        # pad the sample if necessary
        if padding_flag:
            pw = max((self.output_size[1] - image.shape[1]) // 2 + 3, 0)
            ph = max((self.output_size[2] - image.shape[2]) // 2 + 3, 0)
            pd = max((self.output_size[3] - image.shape[3]) // 2 + 3, 0)
        ...
        if w1 is None:
            ...
            d1 = np.random.randint(0, d - self.output_size[3])
        ...
        for key in sample.keys():
            ...
            # item = item.squeeze().numpy()
            if padding_flag:
                item = np.pad(item, [(0, 0), (pw, pw), (ph, ph), (pd, pd)], mode='constant', constant_values=0)

            item = item[0:b, w1:w1 + self.output_size[1], h1:h1 + self.output_size[2], d1:d1 + self.output_size[3]]
...
class ToTensor_Batch(object):
    ...
    def __call__(self, sample):
        ...
        for key in sample.keys():
            ...
            if key == 'image':
                # print(item.max())
                ret_dict[key] = torch.from_numpy(item).unsqueeze(1).float()
            elif key == 'label':
                # item[item>config.num_cls-1]=0
                ret_dict[key] = torch.from_numpy(item).long()
            else:
